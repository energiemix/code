...
# >>> Nieuwe C code in Bereken.Energiemix.Model3 tov versie 6 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
# veranderingen lijken te zittenin het berekenen van verleizen in C. Dat is redelijk complexe code
# die eerst wel gecontroleerd moet worden
...
xwr        = fread(file="KNMI_cabauw_10min_wind.csv")
windcabauw = xts(xwr[, 2:6], as.POSIXct(xwr$index))
...
# wat gebeurd er als we fossiel door waterstof DIRECT vervangen, ipv via elektrificatie?
# mix <- Bereken.Energiemix.H2El(
#   naam                   = "Energiemix 2B",
#   jaarverbruik           = Energieverbruik.NL.kWh, # het totaalverbruik van Nederland
#   waterstofverbruik      = 0,                    # verbruik aan waterstof als fractie van het jaarverbruik. Stroomverbruik is 1-waterstofverbruik.
#   zonnecellen.km2        = 1500,                   # aantal km2 belegt met zonnecellen
#   rendement.zonnecellen  = 0.2,                    # rendement van de zonnecellen, typisch 10-20%
#   windmolens.km2         = 23000,                  # aandeel tov totaal jaarlijks energieverbruik NL
#   separatieafstand       = 7,                      # hoe ver de molens uit elkaar staan in wiekdiameters
#   veldefficientie        = 0.6,                    # de reductie van efficientie van het veld, zie Meneveau, 0.6 bij 7 diam.
#   windtype               = "Enercon-126",          # of "Betz". gebruik theorie of de Enercon-126 om opbrengst te bepalen.
#   opslaggrootte.TWh      = 10,                     # De grootte van de opslag in TWh, hier waterstof!!
#   efficientie.H2.naar.El = 0.5,                    # de efficiency van waterstof omzetten in stroom
#   efficientie.El.naar.H2 = 0.8,                    # de efficiency van stroom omzetten in waterstof
#   startmetvolleaccu      = FALSE,
#   Periode                = "2019"
# )
# 
# Energiemix.Visualisatie3.TWh(mix, visualisatie.tijdschaal = "daily", figuurtitel="Energiemix Test", figuursubtitel="(beschrijf)", figheights=c(1,0))

# t(mixh2$metrics) # print metrics
# 
# # oude situatie, volledige elektrificatie 
# mixoud <- Bereken.Energiemix.Praktijk(
#   naam                   = "Energiemix 2B",
#   jaarverbruik           = Energieverbruik.NL.kWh, # het totaalverbruik van Nederland
#   zonnecellen.km2        = 1500,                   # aantal km2 belegt met zonnecellen
#   rendement.zonnecellen  = 0.2,                    # rendement van de zonnecellen, typisch 10-20%
...
#   windtype               = "Enercon-126",          # of "Betz". gebruik theorie of de Enercon-126 om opbrengst te bepalen. 
#   opslaggrootte.TWh      = 10,                     # De grootte van de opslag in TWh, hier waterstof!!  
#   opslagefficientie      = 0.4,
#   startmetvolleaccu      = FALSE,
#   Periode                = "2019"
# )
# 
# Energiemix.Visualisatie2.TWh(mixoud, visualisatie.tijdschaal = "daily", figuurtitel="Energiemix met opslag & stroomverbruik")
# t(mixoud$metrics) # print metrics
...
MaakResultatenTabelModel2 <- function(mixen) {
  ...
  for (l in 1:length(mixen)) {
    ...
    rw = data.frame(
      ...
      opslagefficientie       = ifelse("opslagefficientie" %in% colnames(mix$metrics), 100*mix$metrics$opslagefficientie, NA),
      eff.verlies             = ifelse("eff.verlies" %in% colnames(mix$metrics), 100*mix$metrics$efficientie.El.naar.H2, NA),
...
MaakVerkorteUitkomsten <- function(results) {
  # Verkorte tabel voor de compacte weergave van niet meer dan een paar scenario's
  # res = Resultatentabel.Energiemixen[6:8, ]
  trans = as.data.frame(t(results))
  empty = trans[1,]; empty[] <- ""  
  verkort = rbind(trans[1,], 
                  empty, trans[2:4,],            # verbruik
                  empty, trans[c(5:6,10),],      # inventaris
                  empty, trans[7:9,],            # capaciteit
                  empty, trans[11:16,],          # productie
                  empty,trans[c(19,22,23),]      # productie-extra
  )
  row.names(verkort)[2] = "Verbruik"
  row.names(verkort)[6] = "Inventaris"
  row.names(verkort)[10] = "Capaciteit"
  row.names(verkort)[14] = "Productie"
  row.names(verkort)[20] = "Verliezen"
  row.names(verkort)[21] = "-"
...
energiemixen = list(
  ...
  zw5050he0100   = Bereken.Energiemix.Model3(naam="zw5050he0100",     jaarverbruik=Energieverbruik.NL.kWh, waterstofverbruik=0, zonnecellen.km2=zonkm2, rendement.zonnecellen=0.2, windmolens.km2=windkm2, separatieafstand=7, veldefficientie=0.6, windtype="Enercon-126", opslaggrootte.TWh=10, eff.R2.O=0.8, eff.O.W=1, eff.O.E=0.5,startmetvolleaccu=FALSE, Periode="2019"),
  ...
  geenopslag   = Bereken.Energiemix.Model3(naam="zon wind, geen opslag", jaarverbruik=Energieverbruik.NL.kWh, waterstofverbruik=0,        zonnecellen.km2=zonkm2, rendement.zonnecellen=0.2, windmolens.km2=windkm2,separatieafstand=7,veldefficientie=0.6,windtype="Enercon-126",opslaggrootte.TWh=0,eff.R2.O=0.8, eff.O.W=1, eff.O.E=0.5,startmetvolleaccu=FALSE, Periode="2019"),
  overcap      = Bereken.Energiemix.Model3(naam="overcapaciteit zon wind, geen opslag", jaarverbruik=Energieverbruik.NL.kWh, waterstofverbruik=0, zonnecellen.km2=2*zonkm2, rendement.zonnecellen=0.2, windmolens.km2=2*windkm2,separatieafstand=7,veldefficientie=0.6,windtype="Enercon-126",opslaggrootte.TWh=0,eff.R2.O=0.8, eff.O.W=1, eff.O.E=0.5,startmetvolleaccu=FALSE, Periode="2019"),
  kernenergie  = Bereken.Energiemix.Model3(naam="kernenergie",           jaarverbruik=Energieverbruik.NL.kWh, waterstofverbruik=0,        zonnecellen.km2=0, rendement.zonnecellen=0.2, windmolens.km2=0,separatieafstand=7,veldefficientie=0.6,windtype="Enercon-126",opslaggrootte.TWh=0,eff.R2.O=0.8, eff.O.W=1, eff.O.E=0.5,startmetvolleaccu=FALSE, Periode="2019"),
  teslas       = Bereken.Energiemix.Model3(naam="zon wind en elektrisch wagenpark",jaarverbruik=Energieverbruik.NL.kWh, waterstofverbruik=0,zonnecellen.km2=zonkm2, rendement.zonnecellen=0.2, windmolens.km2=windkm2,separatieafstand=7,veldefficientie=0.6,windtype="Enercon-126",opslaggrootte.TWh=0.7,startmetvolleaccu=FALSE, Periode="2019"),
  teslas2x     = Bereken.Energiemix.Model3(naam="zon wind en 2x wagenpark",jaarverbruik=Energieverbruik.NL.kWh, waterstofverbruik=0,        zonnecellen.km2=zonkm2, rendement.zonnecellen=0.2, windmolens.km2=windkm2,separatieafstand=7,veldefficientie=0.6,windtype="Enercon-126",opslaggrootte.TWh=1.4,startmetvolleaccu=FALSE, Periode="2019"),
  overkill     = Bereken.Energiemix.Model3(naam="2x zon en wind, 2x wagenpark",jaarverbruik=Energieverbruik.NL.kWh, waterstofverbruik=0,    zonnecellen.km2=zonkm2*2, rendement.zonnecellen=0.2, windmolens.km2=windkm2*2,separatieafstand=7,veldefficientie=0.6,windtype="Enercon-126",opslaggrootte.TWh=1.4,startmetvolleaccu=FALSE, Periode="2019"),
  besparen     = Bereken.Energiemix.Model3(naam="Besparen",              jaarverbruik=Energieverbruik.NL.kWh*0.5, waterstofverbruik=0.2,  zonnecellen.km2=zonkm2, rendement.zonnecellen=0.2, windmolens.km2=windkm2,separatieafstand=7,veldefficientie=0.6,windtype="Enercon-126",opslaggrootte.TWh=0.7,startmetvolleaccu=FALSE, Periode="2019"),
  ...
  besparen.ov  = Bereken.Energiemix.Model3(naam="Besparen overcapa.",    jaarverbruik=Energieverbruik.NL.kWh*0.5, waterstofverbruik=0.2,  zonnecellen.km2=4*zonkm2, rendement.zonnecellen=0.2, windmolens.km2=2*windkm2,separatieafstand=7,veldefficientie=0.6,windtype="Enercon-126",opslaggrootte.TWh=0.7,startmetvolleaccu=FALSE, Periode="2019"),
  besparen.lang    = Bereken.Energiemix.Model3(naam="Besparen lang", jaarverbruik=Energieverbruik.NL.kWh*0.5, waterstofverbruik=0.2, zonnecellen.km2=zonkm2.0119, rendement.zonnecellen=0.2, windmolens.km2=windkm2.0119,separatieafstand=7,veldefficientie=0.6,windtype="Enercon-126",opslaggrootte.TWh=0.7,startmetvolleaccu=FALSE, Periode="2001::2019"),
  besparen.ov.lang = Bereken.Energiemix.Model3(naam="Besparen overc. lang", jaarverbruik=Energieverbruik.NL.kWh*0.5, waterstofverbruik=0.2, zonnecellen.km2=4*zonkm2.0119, rendement.zonnecellen=0.2, windmolens.km2=2*windkm2.0119,separatieafstand=7,veldefficientie=0.6,windtype="Enercon-126",opslaggrootte.TWh=0.7,startmetvolleaccu=FALSE, Periode="2001::2019"),
  ...
  zon.mincap   = Bereken.Energiemix.Model3(naam="zon, min. opslag",          jaarverbruik=Energieverbruik.NL.kWh, waterstofverbruik=0.2,  zonnecellen.km2=35000, rendement.zonnecellen=0.2, windmolens.km2=0,separatieafstand=7,veldefficientie=0.6,windtype="Enercon-126",opslaggrootte.TWh=20,eff.R2.O=0.8, eff.O.W=1, eff.O.E=0.5,startmetvolleaccu=FALSE, Periode="2015::2019"),
  ...
  zon.minopp   = Bereken.Energiemix.Model3(naam="zon, min. oppervlak",       jaarverbruik=Energieverbruik.NL.kWh, waterstofverbruik=0.2,  zonnecellen.km2=6000, rendement.zonnecellen=0.2, windmolens.km2=0,separatieafstand=7,veldefficientie=0.6,windtype="Enercon-126",opslaggrootte.TWh=350,eff.R2.O=0.8, eff.O.W=1, eff.O.E=0.5,startmetvolleaccu=FALSE, Periode="2015::2019"),
  # wind mincap n minopp als bij zon. Gebruik dit voor de vraag: is windeenergie op land zinvol?
  wind.mincap  = Bereken.Energiemix.Model3(naam="wind min. opslag",          jaarverbruik=Energieverbruik.NL.kWh, waterstofverbruik=0.2,  zonnecellen.km2=0, rendement.zonnecellen=0.2, windmolens.km2=1.8*2*windkm2.1519,separatieafstand=7,veldefficientie=0.6,windtype="Enercon-126",opslaggrootte.TWh=20,eff.R2.O=0.8, eff.O.W=1, eff.O.E=0.5,startmetvolleaccu=FALSE, Periode="2015::2019"),
  wind.minopp  = Bereken.Energiemix.Model3(naam="wind min. oppervlak",       jaarverbruik=Energieverbruik.NL.kWh, waterstofverbruik=0.2,  zonnecellen.km2=0, rendement.zonnecellen=0.2, windmolens.km2=1.5*2*windkm2.1519,separatieafstand=7,veldefficientie=0.6,windtype="Enercon-126",opslaggrootte.TWh=350,eff.R2.O=0.8, eff.O.W=1, eff.O.E=0.5,startmetvolleaccu=FALSE, Periode="2015::2019"),

  # pure waterstofmaatschappij versus volledig ge-elektrificeerde maatschappij (MINDER DUIDELIJK)
  elektr.accu         = Bereken.Energiemix.Model3(naam="El accu", jaarverbruik=Energieverbruik.NL.kWh, waterstofverbruik=0, zonnecellen.km2=zonkm2.1519, rendement.zonnecellen=0.2, windmolens.km2=windkm2.1519,opslaggrootte.TWh=0.7,startmetvolleaccu=FALSE, Periode="2015::2019"),
  elektr.h2opslag     = Bereken.Energiemix.Model3(naam="El H2",   jaarverbruik=Energieverbruik.NL.kWh, waterstofverbruik=0, zonnecellen.km2=zonkm2.1519, rendement.zonnecellen=0.2, windmolens.km2=windkm2.1519,opslaggrootte.TWh=50,eff.R2.O=0.8, eff.O.W=1, eff.O.E=0.5,startmetvolleaccu=FALSE, Periode="2015::2019"),
  waterm.h2opslag     = Bereken.Energiemix.Model3(naam="H2 H2",   jaarverbruik=Energieverbruik.NL.kWh, waterstofverbruik=1, zonnecellen.km2=zonkm2.1519, rendement.zonnecellen=0.2, windmolens.km2=windkm2.1519,opslaggrootte.TWh=50,eff.R2.O=0.8, eff.O.W=1, eff.O.E=0.5,startmetvolleaccu=FALSE, Periode="2015::2019"),
  waterm.h2opslag.O.B = Bereken.Energiemix.Model3(naam="H2 H2 OB",jaarverbruik=Energieverbruik.NL.kWh*0.75, waterstofverbruik=1, zonnecellen.km2=1.5*zonkm2.1519, rendement.zonnecellen=0.2, windmolens.km2=windkm2.1519,opslaggrootte.TWh=50,eff.R2.O=0.8, eff.O.W=1, eff.O.E=0.5,startmetvolleaccu=FALSE, Periode="2015::2019"),
  ...
  minkern1 = Bereken.Energiemix.Model3(naam="min. kernenergie 1", jaarverbruik=Energieverbruik.NL.kWh, waterstofverbruik=0.5, zonnecellen.km2=2*zonkm2.1519, rendement.zonnecellen=0.2, windmolens.km2=windkm2.1519,separatieafstand=7,veldefficientie=0.6,windtype="Enercon-126",opslaggrootte.TWh=150, eff.R2.O=0.8, eff.O.W=1, eff.O.E=0.5,startmetvolleaccu=TRUE, Periode="2015::2019"),
  ...
  minkern2 = Bereken.Energiemix.Model3(naam="min. kernenergie 2", jaarverbruik=Energieverbruik.NL.kWh, waterstofverbruik=0.5, zonnecellen.km2=2.1*zonkm2.1519, rendement.zonnecellen=0.2, windmolens.km2=0.5*windkm2.1519,separatieafstand=7,veldefficientie=0.6,windtype="Enercon-126",opslaggrootte.TWh=250, eff.R2.O=0.8, eff.O.W=1, eff.O.E=0.5,startmetvolleaccu=TRUE, Periode="2015::2019"),
  ...
  minkern3 = Bereken.Energiemix.Model3(naam="min. kernenergie 3", jaarverbruik=Energieverbruik.NL.kWh, waterstofverbruik=0.5,  zonnecellen.km2=2.02*zonkm2.1519, rendement.zonnecellen=0.2, windmolens.km2=0.5*windkm2.1519,separatieafstand=7,veldefficientie=0.6,windtype="Enercon-126",opslaggrootte.TWh=250, eff.R2.O=0.8, eff.O.W=1, eff.O.E=0.5,startmetvolleaccu=FALSE, Periode="2015::2019"),
  
  # Wind, zon en kernenergie
  # Kan beperkte inzet van kernenergie de flucuaties van zon en wind opvangen?
  # Dit scenario staat eigenlijk gelijk aan een klein beetje energie uit het buitenland halen.
  windzonkern1 = Bereken.Energiemix.Model3(naam="Wind, zon en kernenergie 1", jaarverbruik=Energieverbruik.NL.kWh, waterstofverbruik=0.5, 
                                       zonnecellen.km2=2*zonkm2.1519, rendement.zonnecellen=0.2, windmolens.km2=windkm2.1519,
                                       separatieafstand=7,veldefficientie=0.6,windtype="Enercon-126",opslaggrootte.TWh=0, 
                                       efficientie.H2.naar.El=0.5, efficientie.El.naar.H2=0.8, startmetvolleaccu=TRUE, Periode="2015::2019"),
  windzonkern2 = Bereken.Energiemix.Model3(naam="Wind, zon en kernenergie 2", jaarverbruik=Energieverbruik.NL.kWh, waterstofverbruik=0.5, 
                                           zonnecellen.km2=4*zonkm2.1519, rendement.zonnecellen=0.2, windmolens.km2=2*windkm2.1519,
                                           separatieafstand=7,veldefficientie=0.6,windtype="Enercon-126",opslaggrootte.TWh=0, 
                                           efficientie.H2.naar.El=0.5, efficientie.El.naar.H2=0.8, startmetvolleaccu=TRUE, Periode="2015::2019"),
  windzonkern3 = Bereken.Energiemix.Model3(naam="Wind, zon en kernenergie 3", jaarverbruik=Energieverbruik.NL.kWh, waterstofverbruik=0.5, 
                                           zonnecellen.km2=2*zonkm2.1519, rendement.zonnecellen=0.2, windmolens.km2=windkm2.1519,
                                           separatieafstand=7,veldefficientie=0.6,windtype="Enercon-126",opslaggrootte.TWh=0.7, 
                                           efficientie.H2.naar.El=0.5, efficientie.El.naar.H2=0.8, startmetvolleaccu=TRUE, Periode="2015::2019"),
  ...
  waterelek1 = Bereken.Energiemix.Model3(naam="waterelek1",          
                                      jaarverbruik=Energieverbruik.NL.kWh, waterstofverbruik=0.5,  zonnecellen.km2=2.02*zonkm2.1519, rendement.zonnecellen=0.2, 
                                      windmolens.km2=0.5*windkm2.1519,separatieafstand=7,veldefficientie=0.6,windtype="Enercon-126",opslaggrootte.TWh=250,
                                      eff.R2.O=0.8, eff.O.W=1, eff.O.E=0.5,startmetvolleaccu=TRUE, Periode="2015::2019"),
  waterelek2 = Bereken.Energiemix.Model3(naam="waterelek2",          
                                         jaarverbruik=Energieverbruik.NL.kWh, waterstofverbruik=1,  zonnecellen.km2=2.02*zonkm2.1519, rendement.zonnecellen=0.2, 
                                         windmolens.km2=0.5*windkm2.1519,separatieafstand=7,veldefficientie=0.6,windtype="Enercon-126",opslaggrootte.TWh=250,
                                         eff.R2.O=0.8, eff.O.W=1, eff.O.E=0.5,startmetvolleaccu=TRUE, Periode="2015::2019"),
  waterelek3 = Bereken.Energiemix.Model3(naam="waterelek3",          
                                         jaarverbruik=Energieverbruik.NL.kWh, waterstofverbruik=0,  zonnecellen.km2=2.02*zonkm2.1519, rendement.zonnecellen=0.2, 
                                         windmolens.km2=0.5*windkm2.1519,separatieafstand=7,veldefficientie=0.6,windtype="Enercon-126",opslaggrootte.TWh=250,
                                         eff.R2.O=0.8, eff.O.W=1, eff.O.E=0.5,startmetvolleaccu=TRUE, Periode="2015::2019")
)

Resultatentabel.Energiemixen = MaakResultatenTabelModel2(energiemixen)
...
# waar komt de energie vandaan? Van een complete installatie!!!


# MaakProductieLijst(Resultatentabel.Energiemixen)

# waterm.h2opslag.O.B = Bereken.Energiemix.Model3(naam="H2 H2 O",jaarverbruik=Energieverbruik.NL.kWh, 
#    waterstofverbruik=0.25, zonnecellen.km2=2*zonkm2, rendement.zonnecellen=0.2, windmolens.km2=windkm2,opslaggrootte.TWh=100,eff.R2.O=0.8, eff.O.W=1, eff.O.E=0.5,startmetvolleaccu=FALSE, Periode="2015::2019")
# energiemixen$besparen_oud$metrics$max.ontlaadvermogen

eendag = Bereken.Energiemix.Model3(naam="min. kernenergie", jaarverbruik=Energieverbruik.NL.kWh, waterstofverbruik=0.2,
                                       zonnecellen.km2=2000, rendement.zonnecellen=0.2,
                                       windmolens.km2=21000,separatieafstand=7,veldefficientie=0.6,windtype="Enercon-126",
                                       opslaggrootte.TWh=0.4, 
                                       eff.R2.O                 = 0.8,                    # 0.8 in het geval van H2-opslag, 1 bij Li-opslag
                                       eff.O.W                  = 1,                      # 1 in het geval van H2-opslag, 0.8 bij Li-opslag
                                       eff.O.E                  = 0.5,                    # 0.5 in het geval van H2-opslag, 0.9 bij Li-opslag
                                       startmetvolleaccu=FALSE, Periode="2007-03-15")

                                       

#                                        
# Energiemix.Visualisatie3.TWh(eendag, visualisatie.tijdschaal = "10min", figuurtitel="Een dag energieopwekking met wind en zon", figuursubtitel="")
