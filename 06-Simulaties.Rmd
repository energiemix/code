...
#### HIER HANDMATIG INSTELLEN ##################################################
# knitr_compile_formaat = "latex" # latex of html
#### HIER HANDMATIG INSTELLEN ##################################################

library(knitr, quietly=TRUE); library(kableExtra, quietly = TRUE); # library(formattable, quietly=TRUE)
library(gt, quietly=TRUE)
# library(flextable, quietly=TRUE); library(huxtable, quietly=TRUE); library(dplyr, quietly=TRUE); library(tibble, quietly=TRUE);

knitr::opts_chunk$set(cache=TRUE)
options(scipen=999)
source("Energietransitie8.R")

# customize figure captions: https://stackoverflow.com/questions/51945079/how-to-change-the-figure-caption-format-in-bookdown

# Tabellen zijn lastig in rmarkdown door de veschillen tussen latex en html output. Hier wordt
# gedetecteerd om welk soort document het gaat in compile-time. Soorten: 'latex' en 'html'
knitr_compile_formaat = knitr::opts_knit$get("rmarkdown.pandoc.to") 
if (knitr_compile_formaat=='latex') corpsgrootte=9
if (knitr_compile_formaat=='html') corpsgrootte=13

# -- hulpfunctie om tabel te genereren
# see new kableExtra: https://cran.r-project.org/web/packages/kableExtra/vignettes/awesome_table_in_html.html
latex.verkorteuitkomstentabel <- function(data, titel="") {
  rn = ncol(data)
  data[1, 1] <- ""
  kbl(data, 
             format=knitr_compile_formaat, # format latex of html 
             booktabs=T,
             caption=titel,
             col.names=NULL, row.names=FALSE, 
             align=c('l', rep('r', rn-1))) %>%
    kable_paper(full_width = F) %>% # bootstrap_options = "striped", 
    kable_styling(font_size = corpsgrootte, position="center") %>%
    # basistabel
    pack_rows("Verbruik", 2, 4) %>%
    pack_rows("Ruimtegebruik", 5, 6) %>%
    pack_rows("Capaciteit", 7, 10) %>%
    pack_rows("Levering", 11, 14) %>%
    pack_rows("Verliezen", 15, 16) %>%
    # uitbreiding tabel
    pack_rows("Draaitijd hulpbron", 17, 18) %>%
    pack_rows("Laadvermogen opslag", 19, 21) %>%
    pack_rows("Inventaris", 22, 24) %>%
    column_spec(1, italic = T, border_right = T) %>%
    column_spec(2:rn, width="2.5cm") %>%
    row_spec(1, italic=T, bold=T) %>%
    kableExtra::footnote(general = "Voor een toelichting op de tabel, zie einde van dit hoofdstuk.",
                         footnote_as_chunk = T, general_title = "", title_format = c("italic"))
  
}

latex.basisuitkomstentabel <- function(data, titel="") {
  rn = ncol(data)
  data[1, 1] <- ""
  kbl(data[1:16,], 
             format=knitr_compile_formaat, 
             booktabs=T,
             caption=titel,
             col.names=NULL, row.names=FALSE, 
             align=c('l', rep('r', rn-1))) %>%
...
# https://www.rstudio.com/blog/great-looking-tables-gt-0-2/ > heeft title + subtitle + bkgrnd
# https://gt.rstudio.com/articles/intro-creating-gt-tables.html
latex.verkorteuitkomstentabel_gt <- function(data, titel) {
  # dit is een aardig eind op weg. Het probleem is dat gt nog geen latex ondersteund.
  colnames(data) = data[1,]
  data2 = data[2:nrow(data),1:ncol(data)]
  colnames(data2)[1] = " "
  gt(data2, rowname_col = "row.names(trans)") %>% # rownames_to_stub = TRUE
      tab_header(title = titel) %>%
      tab_options(table.font.size=9, table.background.color=figuur.achtergrond) %>%
      #cols_label(V1=data[1,2], V2=data[1,3]) %>%
      tab_row_group(label = "Inventaris", rows = 23:25) %>%
      tab_row_group(label = "Laad- en ontlaadvermogens", rows = 20:22) %>%
      tab_row_group(label = "Draaitijd hulpbron", rows = 18:19) %>%
      tab_row_group(label = "Verliezen", rows = 15:17) %>%
      tab_row_group(label = "Levering aan afnemers", rows = 11:14) %>%
      tab_row_group(label = "Capaciteit", rows = 7:10) %>%
      tab_row_group(label = "Ruimtegebruik", rows = 5:6) %>%
      tab_row_group(label = "Verbruik", rows = 2:4) 
      # %>% gtsave(filename="tabel1.png")
}
...
De simulaties nemen zoals eerder het nationale energieverbruik van 2019 als uitgangspunt (`r r0(Energieverbruik.NL.TWh)` TWh). Er wordt aangenomen dat de nationale energievraag constant is. Nederland verbruikt dan continu zo'n `r r0(Energieverbruik.NL.GW)` GW aan energie. Het *vermogen* om Nederland aan de praat te houden is `r r0(Energieverbruik.NL.GW)` GW, de *hoeveelheid energie* die in een jaar wordt verbruikt is `r r0(Energieverbruik.NL.TWh)` TWh.
...
```{r, echo=FALSE, results='asis', fig.align='center', fig.cap="", fig.pos="H", out.width="70%", out.height="70%"} 
# out.width="100%", out.height="100%",
knitr::include_graphics("Model energiemix eenvoudig.jpg")
```
...
```{r dag, echo=FALSE, results='asis', fig.cap="", fig.pos="", fig.height=5, fig.width=8, fig.align='center'}

eendag = Bereken.Energiemix.Model3(naam="voorbeeld", jaarverbruik=Energieverbruik.NL.kWh, waterstofverbruik=0, 
                                       ...
                                       opslaggrootte.TWh=0.4, eff.R1.W = 1, eff.R2.O = 1, eff.H.W = 1, eff.O.W = 1, eff.O.E = 1,
                                       startmetvolleaccu=FALSE, Periode="2007-03-15")
Energiemix.Visualisatie3.TWh(eendag, visualisatie.tijdschaal = "10min", figuurtitel="Figuur 5.2. Een dag energieopwekking met wind en zon", figuursubtitel="")
...
```{r echo=FALSE, results='asis', fig.cap="", fig.pos="", fig.height=5, fig.width=8, fig.align='center'}
Energiemix.Visualisatie3.TWh(energiemixen$geenopslag, visualisatie.tijdschaal = "daily", figuurtitel="Figuur S1. Zon en wind zonder opslag", figuursubtitel="", figheights=c(2,0))

tekort1 = r0(100*energiemixen$geenopslag$metrics$bijgedraaid.pct.van.totaal)
tekort2 = r0(100*energiemixen$overcap$metrics$bijgedraaid.pct.van.totaal)
```
...
res = filter(Resultatentabel.Energiemixen, naam %in% c("S1. zon wind, geen opslag", "S2. overcapaciteit zon wind, geen opslag"))
uitkomsten_go = MaakVerkorteUitkomsten(res)
latex.basisuitkomstentabel(uitkomsten_go, titel = "Zon en wind zonder opslag")
```
...
Hoe groot is de buffer die zou onstaan? Daarvoor moet geschat worden. De duurste variant van de [Tesla model 3](https://en.wikipedia.org/wiki/Tesla_Model_3) heeft een accucapaciteit van 82 kWh. Met dat als uitgangspunt wordt de gecreëerde buffer in totaal `r round(energiemixen$teslas$metrics$opslaggrootte.TWh,1)` TWh groot. Zoals uit bij de behandeling van opslag bleek, zou er qua grondstoffen voor Nederland aan lithium maximaal tweederde dag aan nationaal verbruik (1,3 TWh) beschikbaar zijn. 

Een alternatief is om accucapaciteit te installeren die geheel is toegewijd aan het opvangen van fluctuaties. Dit soort massaopslag wordt inderdaad al gerealiseerd, bijvoorbeeld door [Tesla in Hornsdale](https://en.wikipedia.org/wiki/Hornsdale_Power_Reserve) in Australië. Hornsdale heeft een capaciteit van 194 MWh. Als vervanging van de capaciteit van het wagenpark zou men `r  nicer(TWh.2.Hornsdale(energiemixen$teslas$metrics$opslaggrootte.TWh))` maal de massaopslag van Hornsdale moeten realiseren. Er zou overigens onvoldoende lithium zijn om dit te realiseren als men daarnaast ook nog elektrisch wil rijden.
...
```{r echo=FALSE, results='asis', fig.cap="", fig.pos="", fig.height=5, fig.width=8, fig.align='center'}
Energiemix.Visualisatie3.TWh(energiemixen$teslas, visualisatie.tijdschaal = "daily", figuurtitel="S3. Zon, wind en extra accu's", figuursubtitel="Fluctuaties opgevangen door lithium-accu's", figheights=c(2,0.8))
```

De simulatie (figuur S3) laat zien in welke mate deze buffercapaciteit in staat is de fluctuaties van zon en wind op te vangen. Het blijkt dat de capaciteit aan lithium-accu's ter grootte van het gehele wagenpark van Nederland onvoldoende is. De figuur toont dat de accu vaak leeg of bijna leeg is. Te zien is aan de blauwe gebieden in het jaarverloop dat er frequent nog gebruik wordt gemaakt van een hulpbron om de tekorten aan te vullen. In totaal dekt de accu nu `r r0(100*energiemixen$teslas$metrics$via.opslag.pct.van.totaal)`% van de energielevering en de hulpbron  `r r0(100*energiemixen$teslas$metrics$bijgedraaid.pct.van.totaal)`% (zie tabel 5.2). Het maximale laadvermogen in dit scenario, `r  r0(energiemixen$teslas$metrics$avg.max.laadvermogen*Energieverbruik.NL.GW)` *G*W, blijft ruim binnen de marges van de accu's (0,7 *T*W). Laden is dus niet het probleem, maar capaciteit wel.

Werd in scenario 1 (tabel 5.1) nog 33% van de energiedoor de hulpbron geleverd, nu (in scenario 3) is dat dus gereduceerd tot `r r0(100*energiemixen$teslas$metrics$bijgedraaid.pct.van.totaal)`% . Het tekort halveert dus kennelijk door gebruik te maken van de autoaccu's. Zou het dan zo zijn dat het tekort verdwijnt als deze accucapaciteit twee keer zo groot zou zijn? In scenario S4 is de capaciteit van het wagenpark verdubbeld. Dat vermindert het tekort (en dus het bijdraaien) van `r r0(100*energiemixen$teslas$metrics$bijgedraaid.pct.van.totaal)`% van het jaarlijks verbruik tot `r r0(100*energiemixen$teslas2x$metrics$bijgedraaid.pct.van.totaal)`%. De situatie verandert er niet wezenlijk door.    

Een andere optie is misschien de verdubbeling van de capaciteit van zon en wind in de mix. Om dit te bereiken moet Nederland fictief helemaal worden volgezet met windmolens en zonnecellen bezetten ruim 7% van Nederland. Dat helpt, zoals te zien is in tabel 5.2, scenario 5. Het tekort wordt nu teruggebracht tot `r r0(100*energiemixen$overkill$metrics$bijgedraaid.pct.van.totaal)`%. Nog steeds blijft een hulpbron noodzakelijk. 
...
```{r echo=FALSE, results='asis', fig.cap='', fig.pos='H'}
res = filter(Resultatentabel.Energiemixen, naam %in% c("S3. zon wind en elektrisch wagenpark", "S4. zon wind en 2x wagenpark", "S5. 2x zon en wind, wagenpark"))
uitkomsten_ew = MaakVerkorteUitkomsten(res)
latex.basisuitkomstentabel(uitkomsten_ew, titel="Zon, wind en lithiumaccu's")
```
...
Zou zonne-energie gezien dit alles dan niet een veel efficiëntere keuze zijn? Uit het model blijkt dat `r nicer(100*energiemixen$effzon$metrics$opp.bezetting.zon)`% van NL nodig zou zijn om het jaarverbruik met zonnecellen op te wekken (Tabel 3, S7). Zonne-energie brengt per vierkante kilometer `r nicer(energiemixen$effwind$metrics$opp.bezetting.wind/energiemixen$effzon$metrics$opp.bezetting.zon)` keer meer op. Kunnen we windenergie maar beter laten varen en in plaats daarvan vol inzetten op zonne-energie? Dat is een goede vraag, waar apart op wordt ingegaan. 



```{r echo=FALSE, results='asis'}
res = filter(Resultatentabel.Energiemixen, naam %in% c("S6. Alleen windenergie", "S7. Alleen zonne-energie"))
uitkomsten_eff = MaakVerkorteUitkomsten(res)
latex.basisuitkomstentabel(uitkomsten_eff)
```
...
```{r echo=FALSE, results='asis', fig.cap="", fig.pos="", fig.height=5, fig.width=8, fig.align='center'}
Energiemix.Visualisatie3.TWh(energiemixen$h2opslag, visualisatie.tijdschaal = "daily", figuurtitel="S8. Zon, wind en waterstof", figuursubtitel="", figheights=c(2,0.8))

# alvast voorrekeken, zodat metrics in de tekst kunnen worden gebruikt
mx.laad = energiemixen$h2opslag2l$metrics$avg.max.laadvermogen * TWh.jr.naar.GW(energiemixen$h2opslag2l$metrics$jaarverbruik.TWh)

```

De opslag die hier wordt gesimuleerd is `r r0(energiemixen$h2opslag$metrics$opslaggrootte.TWh)` TWh groot. Dat is een krappe maand aan energieverbruik, best veel dus. De opslag raakt echter niet vol. Hoe komt dat? Er gaat wat energie verloren aan de productie van waterstof. Datzelfde geldt voor het leveren van elektriciteit uit de opslag. Beide hebben te maken met efficiëntieverlies van omzettingen en die zijn niet te vermijden. Daarnaast blijkt dat alle overschot van energie uit wind en zon wordt benut (tabel 5.4, S8, *onbenut*). Dat moet dan betekenen dat er niet voldoende capaciteit is om de opslag te vullen.
...
```{r echo=FALSE, results='asis', fig.cap="", fig.pos="", fig.height=5, fig.width=8, fig.align='center'}
Energiemix.Visualisatie3.TWh(energiemixen$h2opslag2, visualisatie.tijdschaal = "daily", figuurtitel="S9. Veel zon en wind met waterstof", figuursubtitel="", figheights=c(2,0.8))
```
...
Begin met de waterstofproductie, die gebeurt met behulp van electrolysers. Neem aan dat een waterstoffabriek een vermogen van 100 MW heeft ('s werelds grootste heeft een vermogen 20 MW. In Nederland leeft de [ambitie](https://www.technischweekblad.nl/nieuws/uniper-en-haven-overwegen-waterstoffabriek-van-100-mw) om in 2025 naar 100 MW te schalen). Hoeveel zijn er daar van nodig? Om dat te schatten kan er worden gekeken naar het piekvermogen waarmee de waterstofopslag werd geladen. Dat piekvermogen moet door de waterstoffabrieken kunnen worden verwerkt, wil er geen energie verloren gaan. Een conservatieve schatting geeft als piekvermogen `r r0(mx.laad)` GW aan, wat neerkomt op `r nicer(energiemixen$h2opslag2l$metrics$waterstoffabrieken)` waterstoffabrieken (`r r0(mx.laad)` / 0,1). (Het piekvermogen werd geschat door het gemiddelde te nemen van de grootste 20% aan tienminuutsopbrengsten. In de tabel wordt dat aangegeven met 'gemiddeld maximum laadvermogen'. Het absolute maximum wordt ook genoemd, als 'maximum laadvermogen'.)

```{r echo=FALSE, results='asis', fig.cap="", fig.pos="", fig.height=5, fig.width=8, fig.align='center'}
Energiemix.Visualisatie3.TWh(energiemixen$h2opslag2l, visualisatie.tijdschaal = "daily", figuurtitel="S10. Waterstof lange termijn", figuursubtitel="", figheights=c(2,0.8))
```

Waterstof moet ook weer kunnen worden omgezet in elektriciteit. Als er een elektriciteitstekort onstaat, wordt de opslag aangesproken. Voor productie van elektriciteit moet waterstof worden verbrand (een waterstofcentrale kan vergeleken worden met een gascentrale). Het piekvermogen van de elektricteitsvraag bedroeg `r r0(energiemixen$h2opslag2l$metrics$max.ontlaadvermogen.e * TWh.jr.naar.GW(energiemixen$h2opslag2l$metrics$jaarverbruik.TWh))` GW, goed voor `r energiemixen$h2opslag2l$metrics$waterstofcentrales` waterstofenergiecentrales van 3 GW (`r r0(energiemixen$h2opslag2l$metrics$max.ontlaadvermogen.e * TWh.jr.naar.GW(energiemixen$h2opslag2l$metrics$jaarverbruik.TWh))` / 3).  

Compressie en opslag van waterstof vallen buiten de berekeningen van het model. Het piekvermogen dat verwerkt moet worden (`r r0(mx.laad)` GW) is `r  nicer(mx.laad/Energieverbruik.NL.GW)` keer meer (`r r0(mx.laad)` GW / `r r0(Energieverbruik.NL.GW)` GW)dan Nederland continu verbruikt. De waterstof die op deze momenten geproduceerd wordt moet onder de grond worden gepompt. Vergelijk het met spaarbekkens die voldoende pompen en pijpwerk moeten hebben om genoeg water naar boven te kunnen pompen. Is het haalbaar zo'n groot vermogen op tijd te comprimeren en onder de grond te krijgen?
...
```{r echo=FALSE, results='asis'}
res = filter(Resultatentabel.Energiemixen, naam %in% c("S8. Zon, wind en waterstof","S9. Veel zon en wind met waterstof","S10. Waterstof lange termijn"))
uitkomsten_h2 = MaakVerkorteUitkomsten(res)
latex.verkorteuitkomstentabel(uitkomsten_h2, titel="Zon, wind en waterstofopslag")
```
...
Dat betekent dat de problematiek rondom fluctuaties bij kernenergie niet speelt. Uit het oogpunt van energielevering is een energiemix die enkel bestaat uit kernenergie met afstand de meest eenvoudige. Er is geen extra infrastructuur nodig om de energievoorziening op peil te houden. De investering blijft beperkt tot een 100% dekkend vermogen aan kerncentrales. Om Nederland van energie te voorzien volstaan `r  r0(energiemixen$kernenergie$metrics$min.cap.hulpbron*Energieverbruik.NL.GW/3)` kerncentrales van 3 GW (de grootte van de kerncentrale in Doel, net over de Belgische grens). Deze centrales nemen tezamen een ruimte in van `r round(0.8 * Energieverbruik.NL.GW / 3)` km^2^ (berekend aan de hand van de oppervlaktebezetting van de [centrale in Doel](https://nl.wikipedia.org/wiki/Kerncentrale_Doel)).

Kerncentrales produceren elektriciteit. In de energietransitie zullen niet alle processen ge-elektrificeerd kunnen worden. Er blijft een schone brandstof nodig. Dat wordt waarschijnlijk waterstof. Er zal dan infrastructuur nodig zijn voor de productie van waterstof. Het aantal waterstoffabrieken dat nodig is, zal moeten aansluiten op de vraag. In scenario 8 werd er vanuit gegaan dat de maatschappij voor 25% van haar energiegebruik afhankelijk is van waterstof. Om dat verbruik aan te kunnen zijn `r nicer(TWh.jr.naar.GW(energiemixen$kernenergie$metrics$jaarverbruik.TWh)*energiemixen$kernenergie$metrics$waterstofverbruik / 0.1)` waterstoffabrieken à 100 MW nodig. 
...
```{r echo=FALSE, results='asis', fig.cap="", fig.pos="", fig.height=5, fig.width=8, fig.align='center'}
Energiemix.Visualisatie3.TWh(energiemixen$kernenergie, visualisatie.tijdschaal = "daily", figuurtitel="S11. Kernenergie", figuursubtitel="", figheights=c(2,0))
```
...
Toch blijft het om uitval te voorkomen `r r0(100*energiemixen$beperktkern2$metrics$pct.tijd.hulpbron.op.loadfactor90)`% van de tijd nodig om 27 kerncentrales op vol vermogen te laten draaien. Het aantal benodigde centrales wordt niet minder, alleen het aantal dagen dat ze ingezet moeten worden. 
...
```{r echo=FALSE, results='asis'}
res = filter(Resultatentabel.Energiemixen, naam %in% c("S12. Basisscenario beperkt kernenergie","S13. Extra wind en zon","S14. Extra wind, zon en beperkte opslag"))
uitkomsten_h2 = MaakVerkorteUitkomsten(res)
latex.verkorteuitkomstentabel(uitkomsten_h2, titel="Beperkt kernenergie")
```
...
```{r echo=FALSE, results='asis', fig.cap="", fig.pos="", fig.height=5, fig.width=8, fig.align='center'}
Energiemix.Visualisatie3.TWh(energiemixen$zon, visualisatie.tijdschaal = "daily", figuurtitel="S15. Alleen zon", figuursubtitel="", figheights=c(2,0))
```
...
Laten we eerst proberen een schatting te maken voor een scenario waarbij het aantal zonnecellen zo klein mogelijk wordt gehouden. Daar kan het model voor worden gebruikt. De capaciteit van de opslag moet dan zo groot mogelijk zijn. Een inschatting van de maximaal haalbare grootte is lastig, omdat waterstofopslag op deze schaal nog niet eerder werd gerealiseerd. In scenario S16 wordt uitgegaan van 350 TWh. Dat is vergelijkbaar met een half jaar nationaal energieverbruik. (Het is moeilijk zo'n hoeveelheid energie te onderschatten. Dat staat gelijk aan `r nicer(TWh.2.mondiale.cap(350))` keer de *mondiale* opslagcapaciteit in spaarbekkens, die 95% van de mondiale opslag uitmaken.) Met de eis dat de bijdrage van de hulpbron verwaarloosbaar moet blijven, wordt de minimale hoeveelheid zonnecellen bepaald die de energiehuishouding in balans houdt.  

```{r echo=FALSE, results='asis', fig.cap="", fig.pos="", fig.height=5, fig.width=8, fig.align='center'}
Energiemix.Visualisatie3.TWh(energiemixen$zon.minopp, visualisatie.tijdschaal = "daily", figuurtitel="S16. Zon: minimaal ruimtegebruik", figuursubtitel="", figheights=c(2,0.8))
```

In figuur S16 lukt dat. Er wordt geen gebruik gemaakt van een hulpbron. Desondanks kan er altijd aan de energievraag worden voldaan. Om dat te bereiken blijkt er `r r0(energiemixen$zon.minopp$metrics$zonnecellen.km2)` km^2^ aan zonnecellen nodig. Dat is ook zo'n beetje het kantelpunt. Bij minder oppervlak moet de hulpbron al snel worden ingezet. Het model gebruikt `r r0(100*energiemixen$zon.minopp$metrics$opp.bezetting.zon)`% van het landoppervlak voor zonnecellen. Dat komt neer op een kwart van de landbouwgrond (gebruik figuur 2.3 om uw eigen keuze te maken). Hier wordt overigens ook duidelijk dat plaatsing van zonnecellen op daken, waarbij de gebruikte ruimte niet onbruikbaar wordt, weinig zinvol is. Het benodigde oppervlak is daarvoor veel te groot.
...
\bigskip\noindent
De andere knop om aan te draaien, de hoeveelheid zonnecellen, blijkt minder zinvol te zijn. Vergelijk om dat te zien scenario S16 met S17 (tabel 5.6). Om de capaciteit van de opslag een orde van grootte naar beneden te krijgen (van `r energiemixen$zon.minopp$metrics$opslaggrootte.TWh` TWh naar `r energiemixen$zon.mincap$metrics$opslaggrootte.TWh` TWh), zou 85% van Nederland beschikbaar moeten worden gesteld voor zonnecellen. Dat is niet haalbaar. 
...
```{r echo=FALSE, results='asis'}
res6 = filter(Resultatentabel.Energiemixen, naam %in% c("S15. Alleen zon","S16. Zon, ruimte geminimaliseerd","S17. Zon, opslag geminimaliseerd"))
uitkomsten_z = MaakVerkorteUitkomsten(res6)
latex.verkorteuitkomstentabel(uitkomsten_z, titel="Alleen zon")
```
...
```{r echo=FALSE, results='asis', fig.cap="", fig.pos="", fig.height=5, fig.width=8, fig.align='center'}
Energiemix.Visualisatie3.TWh(energiemixen$besparen.opslag, visualisatie.tijdschaal = "daily", figuurtitel="S18. Besparen: minder opslag", figuursubtitel="", figheights=c(2,0.8))
```
...
```{r echo=FALSE, results='asis', fig.cap="", fig.pos="", fig.height=5, fig.width=8, fig.align='center'}
Energiemix.Visualisatie3.TWh(energiemixen$besparen.opper, visualisatie.tijdschaal = "daily", figuurtitel="S19. Besparen: minder ruimtegebruik", figuursubtitel="", figheights=c(2,0.8))
```
...
```{r echo=FALSE, results='asis'}
res7 = filter(Resultatentabel.Energiemixen, naam %in% c("S10. Waterstof lange termijn","S18. Besparen: minder opslag","S19. Besparen: minder ruimtegebruik","S20. Besparen: 25% minder"))
uitkomsten_b = MaakVerkorteUitkomsten(res7)
latex.verkorteuitkomstentabel(uitkomsten_b, titel="Besparen")
```
