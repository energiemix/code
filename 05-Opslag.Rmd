...
# 4 red flags for hydrogen: https://www.youtube.com/watch?v=ge4ux1Y1ric
...
```{r energiedichtheid, include=FALSE}

# constanten
g = 9.8 # gravitatieconstante 9.8 m/s
capaciteit.AA.NiMH.batterij.Wh   = 3.4 # https://en.wikipedia.org/wiki/AA_battery 
capaciteit.AA.batterij.Joule     = capaciteit.AA.NiMH.batterij.Wh * 60 * 60
liter.benzine.Joule              = 32 * 10^6 # https://nl.wikipedia.org/wiki/Energiedichtheid
opp.ijsselmeer.en.markermeer.km2 = 1100 + 700 # https://en.wikipedia.org/wiki/IJsselmeer & https://en.wikipedia.org/wiki/Markermeer

Upot <- function(m,h) { m*g*h }

hoe.hoog.til.je <- function(cap.AA, m=100) { cap.AA / (m*g) }   # Upot = capa.AA = mgh -> h = cap.AA / mg

AA.hoogte = round(hoe.hoog.til.je(capaciteit.AA.batterij.Joule, m=100)/10) * 10
Benzine.hoogte = round(hoe.hoog.til.je(liter.benzine.Joule, m=100000)/10) * 10 

Upot.ijsselmeer = round(opp.ijsselmeer.en.markermeer.km2) * 
                  1000*1000 * # 1000m x 1000m (vierkante km
                  1000 *      # kg, 1000kg per kuub 
                  1 *         # 1m verval
                  10          # de gravitatieconstante
Upot.ijsselmeer.GJ = Upot.ijsselmeer/10^9 

energieopslag.in.sec.tov.verbruik.nl = round(Upot.ijsselmeer.GJ/Energieverbruik.NL.GW)

# hoe hoog moeten dijken om ijsselmeer en markermeer zijn om 1 dag energieverbruik van
# Nederland op te slaan?
# Upot = mg 0.5h, h = 1/0.5h = mg/Upot, h = 2 Upot / mg
hoogte.1dag = (2 * Energieverbruik.NL.PJ*10e15 / 365) / (opp.ijsselmeer.en.markermeer.km2*10e9 * 10)

hoogte.1jaar = (2 * Energieverbruik.NL.PJ*10e15) / (opp.ijsselmeer.en.markermeer.km2*10e9 * 10)

```

Een reden om spaarbekkens als energie-opslagmethode te overwegen, is dat het qua capaciteit wereldwijd de meest gebruikte vorm van energie-opslag is. Spaarbekkens maken 95% van de wereldwijde opslagcapaciteit uit. Dat zal niet voor niets zijn. Wereldwijd is er volgens wikipedia ruim 1.6 TWh beschikbaar ([zie wikipedia/ pumped-storage hydroelectricity](https://en.wikipedia.org/wiki/Pumped-storage_hydroelectricity)). Dat klinkt veel, maar in termen van nationaal energieverbruik (`r r0(Energieverbruik.NL.in.kWh.per.jaar/1e9)` TWh in 2019) is dat slechts `r r0(PSH.mondiaal.aantalurenopslag)` uur (365 dagen * 24 uur * 1.6 TWh / `r r0(Energieverbruik.NL.in.kWh.per.jaar/1e9)` Twh  = `r r0(PSH.mondiaal.aantalurenopslag)` uur). Gemiddeld, vindt wikipedia, is een spaarbekken in een uur of tien te vullen.
...
Het verval van dit spaarbekken is twee meter, als het bekken helemaal vol zit. Het gemiddelde verval van dit enkele spaarbekken is de helft daarvan, één meter. Het IJsselmeer en Markermeer zijn bij elkaar `r round(opp.ijsselmeer.en.markermeer.km2)` km^2^ groot. Een kubieke meter water weegt 1000 kg, dus dit oppervlakte weegt bij één meter diepte $`r round(opp.ijsselmeer.en.markermeer.km2)`$ miljard kg. De opgeslagen energie die dit meer vertegenwoordigt, is daarmee gelijk aan $`r round(opp.ijsselmeer.en.markermeer.km2)`$ miljard kg $\times$ 10 m/s = $`r round(Upot.ijsselmeer.GJ)`$ GJ. Om dat in perspectief te zetten: Nederland gebruikt een continue vermogen van ongeveer `r round(Energieverbruik.NL.GW)` GJ per seconde, dus dit spaarbekken is in staat om `r round(energieopslag.in.sec.tov.verbruik.nl/60)` minuten van ons energieverbuik op te slaan. 

Dat is teleurstellend, want de massa-opslag waarna we op zoek zijn zit meer in de orde van grootte van dagen tot weken, misschien zelfs maanden. Beide meren vertegenwoordigen al een substantieel deel van ons binnenwater, dus heel veel meer oppervlakte is niet realiseerbaar. Als we een *dag* van ons nationaal energieverbruik zouden willen opslaan, dan zou de dijk `r r0(hoogte.1dag)` meter hoog moeten zijn. 
...
# Hoeveel potentiële energie kan er in een bak water worden opgeslagen?
# Upot = mgh
# m = hoogte x breedte x lengte (m) x 1000 (kg), een kuub water is 1000 kg
# g = 10 m/s
# h = hoogte/2 # verval is de helft van de hoogte van de bak


kuub.sogne = 120e3 * 5e3 * 500
massa.sogne = kuub.sogne*1000
Upot = (500/2)*massa.sogne # 250: halve hoogte van de bak
sogne.GJ = Upot/1e9
NL.sec.sogne = sogne.GJ/Energieverbruik.NL.GW
dagdn.verbruik.sogne = NL.sec.sogne/3600/24

# 1600 kilo per kuub aarde

# De energie in een standaard AA-batterij komt overeen met het `r AA.hoogte` meter optillen van een gewicht van 100 kg. Benzine is nog energiedichter: een liter krijgt 100.*000* kilo zo'n `r Benzine.hoogte` meter de lucht in. De energie die kan worden opgeslagen in hoogteverschillen is dus niet erg efficient, maar daar staat tegenover dat de spaarbekkens groot kunnen zijn. 
...
Hoeveel potentiële energie kan er in dit fjord worden opgeslagen? De berekening is in principe dezelfde als net voor het IJsselmeer is gedaan. De massa van het water in het afgedamde fjord is gelijk aan 120.000 m $\times$ 5.000 m $\times$ 500 m $\times$ 1.000 kg = `r round(massa.sogne/1e9)` miljard kg. Bij een verval van 250 meter, de helft van de hoogte van de bak, betekent dat een energieopslag van `r round(sogne.GJ/1e6)` miljoen GJ. 

Dat zijn enorme getallen, maar afgemeten tegen het energieverbruik van Nederland is dat nog steeds relatief. Bij een continue vermogen van `r round(Energieverbruik.NL.GW)` GW, is deze opslag equivalent aan `r round(sogne.GJ/1e6)` miljoen GJ / `r round(Energieverbruik.NL.GW)` GW / 3600 seconden / 24 uur = `r round(dagdn.verbruik.sogne,1)` dagen nationaal energieverbuik. Een blik op de kaart van Noorwegen leert dat de hoeveelheid fjorden van grote omvang eindig is. Misschien dat er nog een handvol te vinden is? Moeten die allemaal worden afgedamd? Als dat al lukt, zouden er nog andere landen in Europa zijn die de Noren zouden vragen om energieopslag, als zij ook fluctuerende bronnen zouden hebben?  
...
``` {r include=FALSE}
# [Battery Storage WIKI](https://en.wikipedia.org/wiki/Battery_storage_power_station)
# [utility scale effciency PSH en accus](https://www.eia.gov/todayinenergy/detail.php?id=46756): 80%
# [Li-ion round trip efficiency 95%.](https://www.nrel.gov/docs/fy19osti/73520.pdf) 
# https://www.fluxpower.com/blog/top-5-factors-that-affect-industrial-battery-efficiency#:~:text=These%20are%20the%20most%20efficient,acid%20drops%20below%2050%25%20efficiency.
# 
# [Overzicht efficientie accutypen](https://en.wikipedia.org/wiki/Comparison_of_commercial_battery_types#Rechargeable_characteristics)
# 
# [Hornsdale Power Reserve](https://en.wikipedia.org/wiki/Hornsdale_Power_Reserve) in Australië, gebouwd door Tesla, heeft een capaciteit van 194 MWh. Dus minder dan 5 sec Nederlands energieverbruik. Ondertussen is [Gateway Energy Storage](https://en.wikipedia.org/wiki/Battery_storage_power_station#Largest_grid_batteries) de grootste accu-opslag ter wereld in de vorm van Lithium-ion accu's met 230 MWh. 
# 
# [Tesla model 3](https://en.wikipedia.org/wiki/Tesla_Model_3) heeft in de grootste variant een accu met een opslagcapaciteit van 82 kWh. 
# 
# [lithium wereldwijde hoeveelheid](https://en.wikipedia.org/wiki/Lithium#Reserves) 86 miljoen ton = 86 miljard kg
# [hoeveel lithium heeft een accu nodig?](https://en.wikipedia.org/wiki/Lithium_battery) The battery requires from 0.15 to 0.3 kg of lithium per kWh. Hoeveel kWh is er dus: 86e9/0.2 = 4.3e11 kWh = 430 =TWh
#vgl: https://www.ultralifecorporation.com/PrivateDocuments/WP_li-ion-vs-lead-acid-WEB_1.pdf

# Betere bron: https://publications.lib.chalmers.se/records/fulltext/230991/local_230991.pdf: 160 gr Li / kWh.

# (Zie ook het leesbare [*A nation-sized battery*](https://dothemath.ucsd.edu/2011/08/nation-sized-battery/), voor een soortgelijke berekening met loodaccu's.)
# 
# utility scale batteries and PHS: (https://www.eia.gov/todayinenergy/detail.php?id=46756

# bron wiki: https://en.wikipedia.org/wiki/Energy_density
specific.energy.waterstof.MJ.kg        = 119.93
specific.energy.benzine.MJ.kg          = 46.6
dichtheid.waterstofgas.1bar.Wh.L       = 2.8
dichtheid.waterstofgas.690bar.Wh.L     = 1250
dichtheid.waterstofgas.vloeibaar.Wh.L  = 2358.6
dichtheid.benzine.Wh.L                 = 9500
# r0(dichtheid.benzine.Wh.L/dichtheid.waterstofgas.1bar.Wh.L/1000)*1000


r0(dichtheid.waterstofgas.690bar.Wh.L/dichtheid.benzine.Wh.L)
r0(dichtheid.benzine.Wh.L/dichtheid.waterstofgas.vloeibaar.Wh.L)

# Hoeveel waterstof moet je per uur kunnen opslaan?
# energy density: https://www.hydrogen.energy.gov/pdfs/review05/st1_satyapal.pdf
# op 200 bar ongeveer 0.5 kWh per liter # Avg.max.laadvermogen is 300 GW,
# dus per uur moet je 300 GWh / 0.5 kWh aan liters wegzetten
# l = 300e9/0.5e3
# kuub = l/1000
...
Waterstof heeft een hoge [energiedichtheid](https://en.wikipedia.org/wiki/Energy_density) per gewicht. Per kilogram levert het `r r0(specific.energy.waterstof.MJ.kg/specific.energy.benzine.MJ.kg)` keer meer energie dan benzine. Waterstofgas heeft echter geen grote dichtheid, waardoor het per volume juist minder energie in zich draagt dan benzine. Per liter is de energiedichtheid van waterstof slechts $\frac{1}{`r r0(dichtheid.benzine.Wh.L/dichtheid.waterstofgas.1bar.Wh.L/1000)*1000`}$ van benzine. Deze lage energiedichtheid valt te verbeteren door het gas te comprimeren. Bij 690 bar bevat het gas $\frac{1}{`r r0(dichtheid.benzine.Wh.L/dichtheid.waterstofgas.690bar.Wh.L)`}$ van de energie van een vergelijkbare hoeveelheid benzine. [Vloeibaar waterstof](https://en.wikipedia.org/wiki/Liquid_hydrogen) heeft de hoogste energiedichtheid, maar bezine blijft ook dan nog een factor `r r0(dichtheid.benzine.Wh.L/dichtheid.waterstofgas.vloeibaar.Wh.L)` energiedichter. Om waterstof vloeibaar te maken, moet het gekoeld worden tot -253° C.  
...
# Energieopslag met behulp van waterstof: electrolyse, opslag in zoutkoepels, terugwinning door verbranden in elektricicteitscentrale. (https://en.wikipedia.org/wiki/Hydrogen_storage#Underground_hydrogen_storage)
# 
# Electrolyse. Polymer electrolyte membrane (PEM) electrolyse efficiency is 80% (https://en.wikipedia.org/wiki/Polymer_electrolyte_membrane_electrolysis#PEM_Efficiency).   
# (https://allesoverwaterstof.nl/s-werelds-grootste-pem-elektrolyser-in-gebruik-genomen/)
#                                                                                
# Ondergrondse opslag. Compressie tot 200 bar kost 2% van de hoeveelheid energie (https://en.wikipedia.org/wiki/Underground_hydrogen_storage). 
# 
# Verbranden. Combined cycle gas turbine efficiency 50%-60%. 
# Brandstofcel: https://nl.wikipedia.org/wiki/Brandstofcel. PEM-fuel cell (Proton-Exchange Membrane), heet verwarrend genoeg ook PEM. Efficiency typisch 50%.
# [The energy efficiency of a fuel cell is generally between 40 and 60%; however, if waste heat is captured in a cogeneration scheme, efficiencies of up to 85% can be obtained.](https://en.wikipedia.org/wiki/Fuel_cell)
# 
# 
# https://en.wikipedia.org/wiki/Polymer_electrolyte_membrane_electrolysis#PEM_Efficiency
# https://en.wikipedia.org/wiki/Hydrogen_production#Industrial_output_and_efficiency
# https://en.wikipedia.org/wiki/Hydrogen_storage#Underground_hydrogen_storage
# https://www.greencarcongress.com/2019/02/20190226-airliquide.html 20 MW
# 
# [Delfzijl](https://www.gasunienewenergy.nl/projecten/djewels)
# [krant ](https://www.dvhn.nl/economie/Bouw-waterstoffabriek-Delfzijl-nog-dit-jaar-25264764.html)
# [Waterstof round trip efficiency 35%.](https://www.nrel.gov/docs/fy19osti/73520.pdf) 
# [Hydrogen or batteries for grid storage? A net energy analysis](https://pubs.rsc.org/en/content/articlehtml/2015/ee/c4ee04041d) Li roundtrip 83% efficient, h2 roundtrip 30% efficient.
# 
# [video H2 red flags](https://www.youtube.com/watch?v=ge4ux1Y1ric)
# [Real Engeneering H2](https://www.youtube.com/watch?v=f7MzFfuNOtY)
```
