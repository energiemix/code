...
knitr::opts_chunk$set(cache=TRUE) # fig.pos="!H" , fig.pos="tbp"
options(scipen=999)
...
# https://nl.wikipedia.org/wiki/Windturbines_in_Nederland -> hoeveel opp. er in de noordzee beschikbaar is
...
```{r hoogtevssnelheid, echo=FALSE, results='asis', fig.cap="", fig.pos="t", fig.height=3, fig.width=5, fig.align='center'}

ggplot(data=gem.windsnelheid.NL.2004.2013, aes(x=hoogte,y=windsnelheid,group=meetmast,color=meetmast)) + 
  theme_maas() +
  labs(title="Figuur 2.1. Hoogte versus windsnelheid", 
           subtitle="Gemiddelde windsnelheid 2004-2013", 
           caption="Data: KNMI") + 
  ylab("Windsnelheid (m/s)") + xlab("Hoogte (m)") +
  geom_line(size=1)
...
```{r wind2007, echo=FALSE, results='asis', fig.cap="", fig.pos="b", fig.height=3, fig.width=5, fig.align='center'}

set2007.betz = Bereken.Opbrengst.Wind2(PERIODICITEIT="daily", periode="2007", soort="Betz") 
gem_windsnelheid = mean(set2007.betz$wind140m)
ggplot(data=set2007.betz, aes(x=Index,y=wind140m)) +
  theme_maas() + 
  labs(title    = "Figuur 2.2. De wind van dag op dag", 
       subtitle = "Etmaalgemiddelde windsnelheid op 140m hoogte") +
  ylab("Windsnelheid (m/s)") + xlab("") +
  #geom_line(size=0.5, color="blue") + # geom_point(color="blue") +
  #geom_hline(yintercept=gem_windsnelheid, linetype="dashed", color = "red") + 
...
```{r Windopbrengst_theorie_en_praktijk, echo=FALSE, results='asis', fig.cap="",fig.height=4, fig.width=6, fig.align='center', fig.pos="t"}

set2007.betz = Bereken.Opbrengst.Wind2(PERIODICITEIT="daily", periode="2007", soort="Betz") 
gem_opbrengst.B = mean(set2007.betz$opbrengst.kWh.km2) / 1e6
set2007.E126 = Bereken.Opbrengst.Wind2(PERIODICITEIT="daily", periode="2007", soort="Enercon-126") 
gem_opbrengst.E = mean(set2007.E126$opbrengst.kWh.km2) / 1e6
combi = cbind(set2007.betz$opbrengst.kWh.km2, set2007.E126$opbrengst.kWh.km2)
colnames(combi) = c("opbrengst.kWh.km2.B","opbrengst.kWh.km2.E")

max.B = max(set2007.betz$opbrengst.kWh.km2)/1e6
max.E = max(set2007.E126$opbrengst.kWh.km2)/1e6

# --- Berekening ruimtegebruik hieronder ---
dta = Bereken.Opbrengst.Wind2(PERIODICITEIT="10min", periode="2006::2015", soort="Enercon-126") 
gem.jaaropbrengst  = sum(dta$opbrengst.kWh.km2)/10
benodigd.oppervlak = round((Energieverbruik.NL.in.kWh.per.dag*365 / 1e9) / (gem.jaaropbrengst/1e9))

ggplot(data=combi) +
  theme_maas() +
  labs(title    = "Figuur 2.3. Energie-opbrengst: theorie en praktijk", 
       subtitle = expression("Dagopbrengst van een km"^2*" windmolens")) +
  ylab(expression("Opbrengst (milj. kWh / km"^2*")")) + xlab("") +
  scale_y_continuous(trans='sqrt', breaks=c(0,0.05,0.1,0.2,0.3,0.5,0.7,1)) + 
  geom_bar(aes(x=Index,y=opbrengst.kWh.km2.B/1e6), stat="identity", color="skyblue", width = 0.05) +
  geom_hline(yintercept=gem_opbrengst.B, linetype="dashed", color = "skyblue") +
  geom_bar(aes(x=Index,y=opbrengst.kWh.km2.E/1e6), stat="identity", color="blue", width = 0.05) +
  geom_hline(yintercept=gem_opbrengst.E, linetype="dashed", color = "blue")
...
gemiddelde jaaropbrengst uit te komen op `r round(gem.jaaropbrengst/1e6)` miljoen kWh per km^2^.
...
overheidsdiensten, alles. Dat verbruik kwam voor 2019 in totaal neer op `r r0(Energieverbruik.NL.TWh)` TWh, zoals werd toegelicht in hoofdstuk 1
(Verbruik).

In dit scenario is er dan, om Nederland van energie te voorzien, een oppervlakte van

$$ \frac {`r round(Energieverbruik.NL.in.kWh.per.dag*365 / 1e9)` \; miljard\; kWh} {`r round(gem.jaaropbrengst/1e6)` \;miljoen\;kWh / km^2} = `r round((Energieverbruik.NL.in.kWh.per.dag*365 / 1e9) / (gem.jaaropbrengst/1e9))` \; km^2$$
\noindent nodig. Nederland beslaat [`r Oppervlakte.Nederland.km2` km^2^](https://nl.wikipedia.org/wiki/Nederland). Om energiedekkend te
worden met windenergie moet dan

$$ \frac {`r round(benodigd.oppervlak)` \; km^2} {`r Oppervlakte.Nederland.km2` \; km^2} = `r round(100 * benodigd.oppervlak / Oppervlakte.Nederland.km2)` \%$$
...
IJsselmeer en Zeeuwse stromen meegerekend. Op dit oppervlak zouden dan zo'n `r round((benodigd.oppervlak/opp.enercon.126.km2)/1000)*1000`
...
`r r1(windsnelheid.op.ashoogte(ashoogte=140, "OWEZ"))` m/s. Cabauw geeft voor de zelfde hoogte
`r r1(windsnelheid.op.ashoogte(ashoogte=140, "Cabauw"))` m/s. Op zee zou men dan een twee keer grotere opbrengst verwachten (`r r1(windsnelheid.op.ashoogte(ashoogte=140, "OWEZ"))`^3^ /
`r r1(windsnelheid.op.ashoogte(ashoogte=140, "Cabauw"))`^3^ =
`r r1(windsnelheid.op.ashoogte(ashoogte=140, "OWEZ")^3 / windsnelheid.op.ashoogte(ashoogte=140, "Cabauw")^3)`), aangenomen dat de windfluctuaties ongeveer
...
```{r opbrengstvariatie2007, echo=FALSE, results='asis', fig.cap="", fig.pos="!t", fig.height=3, fig.width=5, fig.align='center'}

wind2007E = Bereken.Opbrengst.Wind2(PERIODICITEIT="monthly", periode="2007", soort="Enercon-126")
ct1       = cumulatief.tekort2(wind2007E$overschot.pct.jr)
wind2007E = uitbreiden.dataserie.met.tekorten(wind2007E, ct1$benodigde_buffer)
...
plot.tekort.wind.2007E =  grafiek.tekort(wind2007E, 
                                         title      = paste0("Figuur 2.5. Opbrengstvariatie in een jaar"),
                                         subtitle   = paste0("Maandopbrengst als percentage van de jaaropbrengst"),
                                         toontekst  = TRUE,
                                         toonbuffer = FALSE,
                                         rounding=1)
plot.tekort.wind.2007E

opbrengst.jan = wind2007E$productie.pct.jr[1]
overproductie.jan = opbrengst.jan/8.333-1 

opbrengst.okt = wind2007E$productie.pct.jr[10]
onderproductie.okt = abs(opbrengst.okt/8.333-1) 

# rekenvoorbeeld
# round(wind2007E$productie.pct.jr[4:10],1)
opbrengsttekort = sum(round(8.333 - wind2007E$productie.pct.jr[4:10],1))
...
Hoe groot is het overschot? Neem januari. Januari genereert `r round(opbrengst.jan,1)`% van de totale jaaropbrengst. In januari kan
er dus bijna twee keer meer geproduceerd worden dan het gemiddelde van $8\frac{1}{3}$%. Tekorten zijn er ook. Neem oktober, dat een opbrengst heeft van `r round(opbrengst.okt,1)`%. Ten opzichte van het gemiddelde wordt er nu slechts de helft geproduceerd. De opbrengstverschillen zijn dus aanzienlijk. 
...
```{r opbrengstvariatie2007.d, echo=FALSE, results='asis', fig.cap="", fig.pos="b", fig.height=3, fig.width=5, fig.align='center'}

wind2007E.d = Bereken.Opbrengst.Wind2(PERIODICITEIT="daily", periode="2007-03", soort="Enercon-126")
ct2         = cumulatief.tekort2(wind2007E.d$overschot.pct.jr)
wind2007E.d = uitbreiden.dataserie.met.tekorten(wind2007E.d, ct2$benodigde_buffer)

index(wind2007E.d) = as.Date(format(index(wind2007E.d), "%Y-%m-%d"))
plot.tekort.wind.2007E.d =  grafiek.tekort(wind2007E.d, 
                                         title    = paste0("Figuur 2.6. Opbrengstvariatie in een maand"),
                                         subtitle = paste0("Opbrengst als percentage van de jaaropbrengst in 2007"),
                                         toontekst= FALSE, toonbuffer = FALSE)
plot.tekort.wind.2007E.d

opbrengst.1403      = wind2007E.d$productie.pct.jr[14]
onderproductie.1403 = 1-opbrengst.1403/(100/365)

```

Er zijn vier dagen in maart te zien waarin de opbrengst vrijwel nihil is: in de periode van 12 t/m 15 maart wordt weinig geproduceerd. Op 14 maart is er een dagtekort van `r round(100*onderproductie.1403,0)`%: het waait bijna niet. Op deze dag moet de energie dus bijna in z'n geheel ergens anders vandaan komen.
...
```{r opbrengstvariatie2018j, echo=FALSE, results='asis', fig.cap="", fig.pos="t", fig.height=3, fig.width=6, fig.align='center'}

wind2018j = Bereken.Opbrengst.Wind2(PERIODICITEIT="10min", periode="2018", soort="Enercon-126")
ct2       = cumulatief.tekort2(wind2018j$overschot.pct.jr)
wind2018j = uitbreiden.dataserie.met.tekorten(wind2018j, ct2$benodigde_buffer)

#index(wind2007E.d) = as.Date(format(index(wind2007E), "%Y-%m-15"))
plot.tekort.wind.wind2018j =  grafiek.tekort(wind2018j["2018-06"], 
                                           title    = paste0("Figuur 2.7. Opbrengstvariatie Juni 2018"),
                                           subtitle = paste0("Opbrengst als percentage van de jaaropbrengst in 2018"),
                                           toontekst= FALSE, toonbuffer = FALSE)
plot.tekort.wind.wind2018j

gem.opbrengst.10min = 1/(365*24*6)
gem.opbrengst.eerste.10.dgn = mean(wind2018j["2018-06-01::2018-06-10"]$productie.pct.jr)/gem.opbrengst.10min

windstil.lengte = length(which(round(wind2018j["2018-06-01::2018-06-10"]$productie.pct.jr/gem.opbrengst.10min, 5)<5)) / nrow(wind2018j["2018-06-01::2018-06-10"])

```

De fijnmazigheid laat goed zien hoeveel variatie zo'n windmaand eigenlijk heeft. De rode stippellijn vertegenwoordigt de gemiddelde energievraag in 2018 voor een tijdsvak van 10 minuten (ongeveer 0,002% van de jaaropbrengst). Duidelijk is dat veel tijdsvakken onvoldoende opbrengen, namelijk onder de rode stippellijn. Vaak blijft de opbrengst ver onder het gemiddelde. In de eerste tien dagen van de maand komt de opbrengst tot `r r0(gem.opbrengst.eerste.10.dgn)`% van de gemiddelde energievraag. Een groot gedeelte van het energieverbruik, namelijk `r 100-r0(gem.opbrengst.eerste.10.dgn)`%, zal in deze periode dus moeten worden aangevuld.

Er zijn ook regelmatig perioden met windstiltes te tellen: op het oog zijn er een paar te onderscheiden. Telt de computer, dan is het in totaal `r r0(100*windstil.lengte)`% van de tijd windstil (gedefinieerd als minder dan een twintigste van de gemiddelde opbrengst). Er zijn ook langere perioden van meer dan een week waarin het structureel minder waait.
...
In figuur 2.5 kan men een lange windarme periode zien, lopend van april tot en met oktober. April vormt de eerste maand met een tekort (8,3% - 5,6% = 2,7%) en in de daaropvolgende maanden loopt het tekort verder op. In totaal ontstaat zo een 'lopend' tekort in dit jaar van `r opbrengsttekort`% (zie bijlage A.4,
*berekenen van lopend tekort* voor een nadere uitwerking). Dat betekent dat, om de fluctuaties in de stroomproductie voor dit jaar op te vangen, er opslag ter grootte van `r opbrengsttekort`% van de jaarproductie voorhanden had moeten zijn. Een 'accu' ter grootte van `r opbrengsttekort`% van het jaarverbruik had de tekorten kunnen opvangen, tenminste als de 'accu' bij aanvang voldoende gevuld was.

```{r opbrengst20062015, echo=FALSE}

set20062015.E126 = Bereken.Opbrengst.Wind2(PERIODICITEIT="monthly", periode="2006::2015", soort="Enercon-126") 
gem_opbrengst.E = mean(set20062015.E126$opbrengst.kWh.km2) / 1e6
perio = periodiciteit.per.jaar(set20062015.E126)
...
set20062015h.E126 = Bereken.Opbrengst.Wind2(PERIODICITEIT="10min", periode="2006::2015", soort="Enercon-126") 
ct3               = cumulatief.tekort2(set20062015h.E126$overschot.pct.jr)
set20062015h.E126 = uitbreiden.dataserie.met.tekorten(set20062015h.E126, ct3$benodigde_buffer)
set20062015h.E126$buffer = cummax(set20062015h.E126$benodigde_buffer)
benodigde.buffergrootte.20062015Eh.pct.jr = max(max(set20062015h.E126$buffer), max(set20062015h.E126$lopend_overschot))

# Berekening bijdraaien hulpbronnen hoofdscenario
# alle overproductie gaat verloren, dus we kijken alleen naar de verliesuren
gem.10minproductie    = mean(set20062015h.E126$opbrengst.kWh.km2)
verliesvakken.indexen = which(set20062015h.E126$opbrengst.kWh.km < gem.10minproductie)
verliesvakken         = set20062015h.E126$opbrengst.kWh.km2[verliesvakken.indexen]
tekorten              = verliesvakken - gem.10minproductie
# bereken ook het surplus
surplusvakken.indexen = which(set20062015h.E126$opbrengst.kWh.km >= gem.10minproductie)
surplusvakken         = set20062015h.E126$opbrengst.kWh.km2[surplusvakken.indexen]
...
tekort.als.fractie.van.10jaars.productie    = abs(sum(tekorten)/sum(set20062015h.E126$opbrengst.kWh.km2))
overschot.als.fractie.van.10jaars.productie = abs(sum(overschotten)/sum(set20062015h.E126$opbrengst.kWh.km2))
...
```{r infographic_opslag, echo=FALSE, results='asis', fig.cap="", fig.pos="!t", fig.height=5.5, fig.width=7.5, fig.align='center'}

avg.pct = 100/(6*24*365)
dataset = set20062015h.E126 # deze dataset heeft een resolutie van 10min, maar de figuur is per maand.

# genereer de data voor het opslag-scenario om een grafiek te kunnen maken
opslag = dataset$productie.pct.jr - avg.pct # creeer een array die overschot (+) en tekort (-) aangeeft
opslag$wind        = ifelse(opslag$productie.pct.jr<0, opslag$productie.pct.jr+avg.pct, avg.pct)
opslag$bewaard     = ifelse(opslag$productie.pct.jr>0, opslag$productie.pct.jr, 0)
opslag$aangevuld   = ifelse(opslag$productie.pct.jr<0, abs(opslag$productie.pct.jr), 0)

# percentuele opbrengstverdeling op basis van 10min data:
bijgedraaidpct = sum(opslag$aangevuld)/10   # 39%     10: data is 10 jaar lang, beetje een hack
windpct        = sum(opslag$wind)/10          # 61%  
bewaardpct     = sum(opslag$bewaard)/10       # 39% -> bewaard is gelijk aan bijgedraaid

o2 = cbind(apply.monthly(opslag$bewaard, FUN=sum),
           apply.monthly(opslag$aangevuld, FUN=sum),
           apply.monthly(opslag$wind, FUN=sum))
...
rs = rowSums(o2[, 2:3])
for (i in 2:3) o2[,i] = (o2[,i]/rs) * (100/12)
data.opslag = xtsmelt(o2)

colnames(data.opslag) = c("Date","Energieopbrengst","value")
# "italic(Bijschrift)\n First~line \n italic('and a second,')~bold('fancy one') \n 'also,'~integral(f(x)*dx, a, b)~'for good measure'"
plot.opslag <- ggplot(data.opslag, aes(fill=Energieopbrengst, y=value, x=Date)) + 
  geom_bar(position="stack", stat="identity") +
  geom_hline(yintercept=8.333, linetype="dashed", color = "red") +
  theme_maas() + 
  labs(title    = "Figuur 2.8. Simulatie van tien jaar windenergie met opslag", 
       subtitle = "") +
  ylab("Opbrengst (%)") + xlab("") +
  scale_fill_manual(values=c(kleur.opgeslagen, "darkgreen", "#39B54A") ) +
...
opslagbuffer = cbind(apply.monthly(set20062015h.E126$productie.pct.jr, FUN=sum),
                     apply.monthly(set20062015h.E126$benodigde_buffer, FUN=sum))
# Niet alle maanden zijn even lang, dus dit is niet 100% correct, maar voldoende precies voor dit onderzoek.
opslagbuffer$benodigde_buffer = opslagbuffer$benodigde_buffer / (6*24*365/12) # 6: 6x10min = 1 uur 

plot.opslagbuffer = grafiek.buffercapaciteit2(opslagbuffer, title="Laadtoestand denkbeeldige accu")
grid.arrange(plot.opslag, plot.opslagbuffer, ncol = 1, heights = c(6, 4))
...
hulpbron$wind       = ifelse(hulpbron$productie.pct.jr<0, hulpbron$productie.pct.jr+avg.pct, avg.pct)
...
windpct    = sum(hulpbron$wind)/10       # 61%  
...
h2 = cbind(apply.monthly(hulpbron$ongebruikt, FUN=sum),
           ...
           apply.monthly(hulpbron$wind, FUN=sum))
...
ggplot(data.hulpbron, aes(fill=Energieopbrengst, y=value, x=Date)) + 
  ...
  geom_hline(yintercept=8.333, linetype="dashed", color = "red") +
  theme_maas() + 
  labs(title    = "Figuur 2.9. Simulatie van tien jaar windenergie met hulpbron") +
  ylab("Opbrengst (%)") + xlab("") +
  scale_fill_manual(values=c("#999999", "#56B4E9",kleur.wind) ) +
  theme(legend.position="top", legend.title = element_blank(), axis.title.x = element_blank())
...
In dit scenario blijkt windenergie `r round(100*(1-tekort.als.fractie.van.10jaars.productie))`% van de jaarlijkse energievraag te leveren en de hulpbron moet dus `r round(100*tekort.als.fractie.van.10jaars.productie)`% van de benodigde energie bijdraaien. 
